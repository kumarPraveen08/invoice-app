...
export default function NewClientScreen() {
  ...
  const importFromContacts = async () => {
    try {
      ...
      if (status !== "granted") {
        showSnackbar(
          "Contacts permission is off. Enable it in system Settings → Apps → Invoice App → Permissions.",
        );
        return;
      }
      ...
      const nextAddress = postal
        ...
        : "";
      const nextImage = contact.image?.uri ?? contact.rawImage?.uri ?? null;
      ...
      const needsRebuild =
        /ExpoContacts|native module|READ_CONTACTS/i.test(message) ||
        message.includes("Cannot find native module");
      showSnackbar(
        Platform.OS === "web"
          ? "Contact import is not available on web."
          : needsRebuild
            ? "Rebuild the app once (pnpm android) so contacts permission is installed."
            : message,
      );
    }
  };
