...
export function InvoiceTemplatePreview({
  ...
  const text = (size: number, weight: '400' | '600' | '700' = '400', color = ink) => ({
    fontFamily: ff,
    fontSize: size,
    fontWeight: weight,
    color,
  ...
  return (
    ...
    >
      {layout === 'modern' ? (
        <View style={[styles.modernHeader, { backgroundColor: accent, margin: -pad, marginBottom: pad, padding: pad }]}>
          <View style={styles.headerRow}>
            {renderLogo(true)}
            <View style={{ flex: 1 }}>
              <Text style={text(14, '700', '#FFFFFF')}>{biz}</Text>
              {fields.businessAddress && business.address ? (
                <Text style={text(9, '400', 'rgba(255,255,255,0.85)')} numberOfLines={2}>
                  {business.address}
                </Text>
              ) : null}
            </View>
        ...
        <View style={[styles.headerRow, { marginBottom: dense ? 8 : 12 }]}>
      ...
      {layout === 'classic' ? (
        <View style={{ height: 2, backgroundColor: accent, marginBottom: dense ? 8 : 12 }} />
      ) : null}

      {layout === 'modern' ? (
        <View style={[styles.metaRow, { marginBottom: dense ? 8 : 12 }]}>
          <View>
        ...
        <View style={{ marginBottom: dense ? 8 : 12 }}>
          <Text style={text(8, '600', muted)}>BILL TO</Text>
          <Text style={text(10, '600')}>Acme Studio</Text>
          <Text style={text(9, '400', muted)}>Jane Cooper</Text>
        </View>
      ...
      <View style={[styles.tableHead, { borderBottomColor: line, marginBottom: 4 }]}>
        <Text style={[text(8, '600', muted), { flex: 1 }]}>ITEM</Text>
        <Text style={[text(8, '600', muted), { width: 28, textAlign: 'right' }]}>QTY</Text>
        <Text style={[text(8, '600', muted), { width: 64, textAlign: 'right' }]}>AMOUNT</Text>
      </View>
      {SAMPLE_LINES.map((lineItem) => (
        <View key={lineItem.name} style={[styles.tableRow, { paddingVertical: dense ? 3 : 5 }]}>
          <Text style={[text(9, '400'), { flex: 1 }]} numberOfLines={1}>
            {lineItem.name}
          </Text>
          <Text style={[text(9, '400', muted), { width: 28, textAlign: 'right' }]}>
            {lineItem.qty}
          </Text>
      ...
      <View style={[styles.totals, { borderTopColor: line, marginTop: dense ? 6 : 10 }]}>
      ...
      {(fields.notes || fields.terms || fields.bankDetails || fields.signature) && (
          ...
          {fields.signature ? (
            <View style={{ alignItems: 'flex-end', marginTop: 4 }}>
              {branding.signatureUri ? (
                ...
                <View style={[styles.signatureLine, { borderBottomColor: muted }]} />
              )}
